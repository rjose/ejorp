...
;; All of our tests will assume a current date of Jan 3, 2012.
(def cur-w (current-week "Jan 3, 2012"))
...
(fact (:end-date cur-w) => (. week-fmt parseDateTime "Jan 8, 2012"))
...
(fact (count (:tasks (first sched1))) => 2)

;; When we get the schedule as a string, it should be correct.
(def expected1 (str "Jan 8, 2012:\n"
                    "\t- Task one: 4P\n"
                    "\t- Task two 8P\n\n"))
